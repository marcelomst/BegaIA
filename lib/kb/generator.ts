// Path: /root/begasist/lib/kb/generator.ts
...
export function genCancellationPolicy(p: Profile) {
  const policy = p.policies?.cancellation || 'Las reservas pueden cancelarse sin cargo hasta 48 horas antes del check-in. Dentro de las 48 horas o en caso de no presentarse (no-show), se cobra el equivalente a 1 noche.';
  const lines = [policy];
  // 🔁 categoría alineada al grafo/contrato: cancel_reservation
  return header('Política de cancelación', 'cancel_reservation', 'cancellation_policy', 'Reglas y plazos de cancelación de reservas.') + bullets(lines) + fuentes();
}
...
export function generateKbFilesFromProfile(p: Profile): Record<string, string> {
  ...
  // 🔒 Convención estable: category/promptKey.lang.txt
  files['retrieval_based/kb_general.es.txt'] = genKbGeneral(p);

  files['retrieval_based/room_info.es.txt'] = genRoomInfo(p);
  files['retrieval_based/room_info_img.es.txt'] = genRoomInfoImg(p);
  ...
  files['cancel_reservation/cancellation_policy.es.txt'] = genCancellationPolicy(p);
  return files;
}
