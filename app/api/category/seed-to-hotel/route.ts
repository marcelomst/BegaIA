...
export async function POST(req: NextRequest) {
    ...
    const overrideTitle: string | undefined = typeof body?.overrideTitle === "string" ? body.overrideTitle : undefined;
    const overrideBody: string | undefined = typeof body?.overrideBody === "string" ? body.overrideBody : undefined;
    ...
    // 2) Fallback legacy: seed local (mientras exista transición)
    if (!template) {
        try {
            ...
            // ignorar: seguimos con DB/CQL
        }
    }

    // 3) Fallback category_registry (Document API o CQL)
    if (!template) {
    ...
    // Si el body viene override, lo usamos aunque no exista template
    const effectiveBody = overrideBody ?? template?.body;
    const effectiveTitle = overrideTitle ?? template?.title;
    if (!effectiveBody) {
        return NextResponse.json({ error: `Template inexistente para ${categoryId} (${lang}). Agrega seeds/category_registry.json o envía overrideBody.` }, { status: 404 });
    }
