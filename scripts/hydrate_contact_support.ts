#!/usr/bin/env tsx
/**
 * Hydrate support/contact_support from seeds using a provided config JSON (inline or @path).
 * Run with:
 *   pnpm exec tsx scripts/hydrate_contact_support.ts --lang es --config '{"contact":{"phone":"+54 11 4444 4444","whatsapp":"+54 9 11 4444 4444","email":"recepcion@hotel.com","hours":"24/7","otherChannels":"Instagram, Facebook"}}'
 *   pnpm exec tsx scripts/hydrate_contact_support.ts --lang pt --config @/path/to/config.json
 */

import { readFileSync } from "node:fs";
import * as path from "node:path";

const CATEGORY_ID = "support/contact_support" as const;
...
function parseArgs(argv: string[]) {
    ...
    for (let i = 2; i < argv.length; i++) {
        ...
        // positional fallbacks: [lang] [config]
