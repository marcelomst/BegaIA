...
async function hasAnyGuestAliasByHotel(client: Client, table: "guest_aliases", hotelId: string): Promise<boolean> {
...
async function hasAnyGuestAliasesByGuestForCandidates(client: Client, hotelId: string, guestIds: string[]): Promise<boolean | null> {
...
async function hasAnyGuestAliasesByGuestWithFiltering(client: Client, hotelId: string): Promise<boolean> {
  const query = "SELECT alias FROM guest_aliases_by_guest WHERE hotelid = ? LIMIT 1 ALLOW FILTERING";
  const res = await client.execute(query, [hotelId], { prepare: true });
  return Boolean(res.first());
}

async function listGuestIdsFromGuestAliases(client: Client, hotelId: string): Promise<string[]> {
...
async function main() {
  ...
  try {
    ...
    const guestIdCandidates = args.hotel
      ? Array.from(new Set([
          ...(await guestsCol.find({ hotelId: args.hotel }, { projection: { guestId: 1 } }).toArray())
            ...
            .filter(Boolean),
          ...(await listGuestIdsFromGuestAliases(cqlClient, args.hotel)),
        ]))
      : [];
    ...
    if (!args.only || args.only === "guest_aliases") {
      targetTables.push({
        ...
        run: async () => {
          ...
          await cqlClient.execute("DELETE FROM guest_aliases WHERE hotelid = ?", [args.hotel], { prepare: true });
      ...
      targetTables.push({
        ...
        run: async () => {
          ...
          for (const guestId of guestIdCandidates) {
            await cqlClient.execute("DELETE FROM guest_aliases_by_guest WHERE hotelid = ? AND guestid = ?", [args.hotel, guestId], {
              prepare: true,
            });
            deletedPartitions += 1;
          }
    ...
    if (args.hotel) {
      try {
        const hasGuestAliases = await hasAnyGuestAliasByHotel(cqlClient, "guest_aliases", args.hotel);
        const hasGuestAliasesByGuest = await hasAnyGuestAliasesByGuestForCandidates(cqlClient, args.hotel, guestIdCandidates);
        const hasGuestAliasesByGuestFiltering =
          hasGuestAliasesByGuest === null
            ? await hasAnyGuestAliasesByGuestWithFiltering(cqlClient, args.hotel)
            : null;
    ...
    await cqlClient.shutdown().catch((err) => {
      console.warn("⚠️ No se pudo cerrar cliente Cassandra:", err);
    });
  }
}
