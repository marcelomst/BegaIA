...
describe("messageHandler post-booking reservation snapshot", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it.each([
    ["cuál es mi reserva", /resumen de tu reserva|código: res-ab12cd|21\/03\/2026 → 25\/03\/2026|huéspedes: 2/i],
    ["me recordás la reserva", /resumen de tu reserva|marcelo martinez|habitación: doble/i],
    ["qué fechas reservé", /21\/03\/2026 → 25\/03\/2026/i],
    ["cuántos huéspedes puse", /2 huésped/i],
  ])("con reserva confirmada, %s responde snapshot determinista", async (content, expected) => {
    const conversationId = `conv-postbooking-snapshot-${content}`;
    ...
    await handleIncomingMessage(msg(String(content), conversationId), { mode: "automatic", sendReply });
