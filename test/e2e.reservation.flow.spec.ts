...
describe("e2e reservation flow (single-slot Q, guests and checkout mapping)", () => {
    beforeEach(() => {
        ...
        // Mock robusto: nunca devuelve pregunta 'undefined', vacía o incoherente
        (fillSlotsWithLLM as any).mockImplementation((input: string, lang: string, opts: any) => {
            // Simula acumulación real de slots
            const prev = opts?.prevSlots || {};
            let partial = { ...prev };
            // Simula lógica de wiring real: mergea slots y pregunta coherente
            if (input === "Marcelo Martinez") {
                partial.guestName = "Marcelo Martinez";
                return { need: "question", partial, question: "¿Cuál es el tipo de habitación que preferís?" };
            }
            if (input === "doble") {
            ...
            if (input === "2") {
                partial.numGuests = 2;
                // Cuando todos los slots están, devuelve need: none
                return { need: "none", slots: { ...partial } };
            }
            if (input === "CONFIRMAR") {
                // Simula confirmación, no pregunta nada
                return { need: "none", slots: { ...partial } };
            }
            // Fallback: nunca pregunta 'undefined'
            return { need: "question", partial, question: "¿Cuál es el tipo de habitación que preferís?" };
        });
    });
